...
class App extends React.Component {
  ...
  handleComplete = text => {
    confirm({
      title: '系统温馨提示',
      content: `您确定要把编号为${text.id}的任务设置为已完成吗？`,
      onOk: async () => {
        let data = await api.task.completeTask(text.id)
        if (parseInt(data.code) === 0) {
          message.success('恭喜您！任务已经设置为已完成')
          this.props.queryAll()
          return
        }
        message.error('很遗憾！任务设置失败，请稍后再试~~')
      }
    })
  }
  handleDelete = text => {
    confirm({
      title: '系统温馨提示',
      content: `您确定要把编号为${text.id}的任务删除吗？`,
      onOk: async () => {
        let data = await api.task.removeTask(text.id)
        if (parseInt(data.code) === 0) {
          message.success('恭喜您！任务已删除~~')
          this.props.queryAll()
          return
        }
        message.error('很遗憾！任务删除失败，请稍后再试~~')
      }
    })
  }
  handleTag = index => {
